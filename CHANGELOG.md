...
* **A CI Integration guide** covering GitLab CI and GitHub Actions. Vertagus drives source control through the `git` executable and calls no host's API, so it already runs on any platform; what varies is the checkout each CI system provides. The guide covers push credentials — notably that GitLab's default `CI_JOB_TOKEN` cannot push to the repository — protected tags, clone depth, and detached HEAD. See `docs/ci-integration.md`.

**Fixes.**

* **Dependabot pull requests no longer fail the version check on their branch name.** `check-version` ends in a catch-all step that fails any branch not prefixed `release/`, `hotfix/`, `feature/` or `nobump/`. Dependabot names its own branches, so every bump touching `pyproject.toml` failed there -- with neither version check having run, since both are gated on the same prefixes. `dependabot/*` is now treated as `nobump/*`: a dependency bump does not change the project's own version.
* **Pull requests that change only the lockfile, the workflows, or the tests now run CI.** The `on-pr` path filter listed `src/**`, `pyproject.toml` and `vertagus.toml`, so a dependency or action bump -- which touches none of those -- merged with no tests and no lint having run. The filter now covers `uv.lock`, `.github/workflows/**` and `test/**`. It also named `vertagus.toml`, a path that does not exist in this repository; edits to `vertagus.yaml` triggered nothing and now do.
* **`create-tag` now pushes only the tag it created, to the configured remote.** It ran a bare `git push --tags`, which published every tag that happened to be in the local clone — a real hazard on a reused CI build directory — and ignored `remote_name`, pushing to the current branch's upstream instead. `delete_tag` shed a trailing `push --tags` for the same reason; its explicit `push --delete` was already the whole job.
* **The `semver` bumper no longer silently reports no commits on a shallow clone.** The highest version is resolved by querying the remote, so it can name a tag that a shallow CI checkout — GitLab CI defaults to `GIT_DEPTH: 20`, `actions/checkout` to `fetch-depth: 1` — does not have locally. Reading that tag's date failed, and the bumper saw an empty commit list and computed no bump. Vertagus now fetches the single missing tag and continues. The commit range it walks is still bounded by the clone, so full history remains the right setting; when the tag cannot be resolved at all, the error now names the shallow clone as the likely cause.
* **A rejected push now explains itself.** Git's stderr for a credentials or branch-protection failure rarely says which it was, so the error raised from a failed push carries the common causes: read-only CI credentials, a protected tag pattern, or a tag already present at another commit.
